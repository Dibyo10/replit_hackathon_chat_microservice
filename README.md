...
This microservice lets your team design APIs just by talking — then automatically generates production-grade artifacts like:

- `openapi.yaml` (API Spec)
- `rules.md` (Design Conventions)
- `schema.json` (Data Model Definitions)

It’s basically an **AI API architect**:  
...
And once you’re done — you type `GENERATE`, and the bot spits out all the files you need.
...
> 💡 **Think ChatGPT — but instead of random text, it gives you a clean, validated OpenAPI contract.**
...
Frontend (test.html / real app)
        ...
        ▼
[ FastAPI Backend ]
├── /api/chat/stream       →  Gemini 1.5 Flash (streaming chat)
├── /api/generate          →  Gemini 2.5 Pro (file generation)
├── /api/files/{id}/openapi
├── /api/files/{id}/rules
└── /api/files/{id}/schema
        │
        ▼
...
| Concept        | Meaning                                                                                       |
| -------------- | -------------------------------------------------------------------------------------------- |
| Session        | A user’s chat context, saved in Redis (not memory). Contains the full chat + generated files.|
...
| Artifacts      | `openapi.yaml`, `rules.md`, and `schema.json` — all generated by Gemini Pro.                 |
...
app/
 ...
 │   └── prompts.py              # System persona prompt (Contract-first architect)
 ├── models/
 │   └── schemas.py              # Pydantic models for requests/responses
 ├── routers/
 │   ├── chat.py                 # /api/chat/stream (Gemini Flash chat)
 │   └── generation.py           # /api/generate + /api/files/*
 ├── services/
 │   ├── gemini_service.py       # Gemini API wrappers
 │   └── session_manager.py      # Session handling (Redis-backed)
 └── utils/
     └── parser.py               # Extracts files from Gemini’s response
```
...
Example:
```
User: We’re building an inventory management API.
Bot: Please define the core entities — Products, Warehouses, or both?
User: Products and Warehouses. UUID IDs, JWT Auth.
```

→ Stored in `session.history` (in Redis).
...
- **Response (streamed):**
    ```
    Bot: Define your core entities — Task, User, Project?
    Bot: What is your ID format?
    [STREAM_END]
    ```

#### 2️⃣ POST `/api/generate`
...
#### 3️⃣ `/api/files/{session_id}/openapi`
- Returns openapi.yaml (raw YAML).

#### 4️⃣ `/api/files/{session_id}/rules`
- Returns rules.md (Markdown).

#### 5️⃣ `/api/files/{session_id}/schema`
...
If you’re the frontend dev, your workflow:

- Call `/api/chat/stream` → stream and show tokens in chat UI.
- When response contains "Ready to generate" → call `/api/generate`.
- Display the returned file contents or let users download them.
- *Optional*: fetch files separately via `/api/files/...` endpoints.

> ✅ Never expose the Gemini API key — it’s server-side only.
...
- Treat it as a microservice — either deploy separately or as a submodule.
...
- **Contract-First** → No code before API spec
- **Deterministic AI** → Same inputs → same artifacts
- **Decoupled architecture** → Chat, generation, parsing, and storage are modular
- **Production-focused** → Redis-backed, scalable, and clean
- **Developer-friendly** → Explicit, predictable, and minimal surprises
